"""Independent checker for the complete D=4 selection sweep."""
...
def main() -> int:
    ...
    if (
        protocol["delay_factor"] != 4
        or protocol["time_cap"] != 8_000.0
    ...
    for item in configurations:
        if item["time_cap"] != 8_000.0 or item["target_accuracy"] != 0.70:
    ...
    if result["verdict"] != "BLOCKED":
        errors.append("D=4 selection sweep must not claim the full result")
    print(
        json.dumps(
            {
                "event": "CLAIM6_D4_SWEEP_INDEPENDENT_CHECK",
